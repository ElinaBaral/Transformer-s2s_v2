...
class LJDatasets(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'mel': mel, 'mel_input':mel_input, 'pos_mel':pos_mel}
        return sample
...
def collate_fn_transformer(batch):
    ...
    if isinstance(batch[0], collections.Mapping):
        ...
        mel = [i for i, _ in sorted(zip(mel), key=lambda x: x[1], reverse=True)]
        mel_input = [i for i, _ in sorted(zip(mel_input), key=lambda x: x[1], reverse=True)]
        #pos_text = [i for i, _ in sorted(zip(pos_text, text_length), key=lambda x: x[1], reverse=True)]
        pos_mel = [i for i, _ in sorted(zip(pos_mel), key=lambda x: x[1], reverse=True)]
        ...
        return t.FloatTensor(mel), t.FloatTensor(mel_input), t.LongTensor(pos_mel)
